...
class ModelBuildingStrategy(ABC):
    @abstractmethod
    def build_and_train_model(self, X: pd.DataFrame, y: pd.Series) -> ClassifierMixin:
        pass
...
class MultinomialNBStrategy(ModelBuildingStrategy):
    def build_and_train_model(self, X: pd.DataFrame, y: pd.Series) -> ClassifierMixin:
        ...
        clf_model = MultinomialNB(alpha=1, fit_prior=False)
        clf_model.fit(X, y)
...
class ModelBuilder:
    ...
    def build_model(self, X: pd.DataFrame, y: pd.Series) -> ClassifierMixin:
        return self._strategy.build_and_train_model(X, y)
...
def main():
    ...
    clf = model_builder.build_model(X_train, y_train)
